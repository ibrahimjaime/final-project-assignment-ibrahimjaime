...
META_OPENEMBEDDED_DIR="$REPO_ROOT/meta-openembedded"
META_OE_DIR="$META_OPENEMBEDDED_DIR/meta-oe"
META_PYTHON_DIR="$META_OPENEMBEDDED_DIR/meta-python"
...
# meta-oe provides libserialport, which data-logger's recipe DEPENDS on
# (serial_reader.c links against it). Check the submodule populated
# correctly, same as the poky check above — without this, add-layer
# below would fail with a much less obvious "layer.conf not found" error.
if [ ! -f "$META_OE_DIR/conf/layer.conf" ]; then
echo "ERROR: $META_OE_DIR/conf/layer.conf not found."
echo "The meta-openembedded submodule did not populate correctly — check 'git submodule status' above for errors."
exit 1
fi
...
# meta-oe and meta-python (both from the meta-openembedded submodule)
# must be added before meta-data-logger builds, since data-logger's
# recipe DEPENDS on libserialport, which meta-oe provides. meta-oe
# itself requires meta-python to be present.
if ! bitbake-layers show-layers | grep -q "meta-python"; then
echo "Adding meta-python layer"
bitbake-layers add-layer "$META_PYTHON_DIR"
else
